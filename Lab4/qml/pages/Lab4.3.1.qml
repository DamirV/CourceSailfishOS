...
Page {
    ...
    SilicaFlickable {
        ...
        Column {
            ...
            Button{
                x: Theme.horizontalPageMargin
                text: qsTr("Ввести текст")
                color: Theme.secondaryHighlightColor
                onClicked: {
                    var dialog = pageStack.push(Qt.resolvedUrl("Lab4.3.2.qml"), {firstTextField: qsTr("")});
                   dialog.accepted.connect(function(){label.text = qsTr(dialog.firstTextField)})
                }
            }
